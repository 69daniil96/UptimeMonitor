...
Минималистичное полупрозрачное окно, которое сидит на рабочем столе и показывает статусы хостов. Умеет определять **DPI-блокировки** (когда TCP-соединение есть, но сервис заблокирован провайдером на уровне TLS). Если хост падает или заблокирован, а окно перекрыто полноэкранным приложением — автоматически всплывает поверх всех окон.

| Статус         | Цвет   | Значение                                 |
| -------------- | ------ | ---------------------------------------- |
| ● ONLINE       | 🟢     | TCP + TLS — сервис полностью доступен    |
| ⚠ DPI BLOCK    | 🟡     | TCP ок, TLS заблокирован — DPI-блокировка |
| ● OFFLINE      | 🔴     | TCP не подключается — хост недоступен    |

| Режим            | Условие                           | Поведение                            |
| ---------------- | --------------------------------- | ------------------------------------ |
| **Нормальный**   | Рабочий стол виден                | Все хосты, обычное окно              |
| **Алерт**        | Окно перекрыто + есть проблемные  | Поверх всех окон, только проблемные  |
...
| ---------------- | ------------------- | ----------------------------------------------- |
| `NetworkWorker`  | Фоновый (`QThread`) | TCP + TLS проверка хостов                       |
...
### Двухуровневая проверка доступности

Для хостов на порту 443 (HTTPS) используется двухуровневая проверка:

```
 TCP connect (socket)        TLS handshake (ssl)
  ├─ ❌ Fail → OFFLINE       ├─ ❌ Fail → DPI BLOCK
  └─ ✅ OK ──────────────►   └─ ✅ OK  → ONLINE
```

**Зачем?** DPI (Deep Packet Inspection) блокирует сервисы не на уровне TCP, а на уровне TLS — читая поле SNI в ClientHello. TCP-соединение проходит успешно, но TLS handshake сбрасывается провайдером. Без TLS-проверки заблокированный сервис показывался бы как ONLINE.

Для портов ≠ 443 (например DNS на 53) используется только TCP-проверка.
